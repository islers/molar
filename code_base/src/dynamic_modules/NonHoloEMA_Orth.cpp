...
#include "NonHoloEMA_Orth.h"


NonHoloEMA_Orth::NonHoloEMA_Orth( Ptr<GenericObject> _objectPointer ):FilteredDynamics(_objectPointer)
{
	pPosAlpha = 0.6;
	pAngAlpha = 0.6;
	pSwitchAlpha = 0.1;
...
NonHoloEMA_Orth::~NonHoloEMA_Orth(void)
{
}


Ptr<GenericObject::Dynamics> NonHoloEMA_Orth::sfmCreator( Ptr<GenericObject> _objectPointer )
{
	return new NonHoloEMA_Orth(_objectPointer);
}


void NonHoloEMA_Orth::setOptions( GenericMultiLevelMap<string>& _dynamicsOptions )
{
...
void NonHoloEMA_Orth::getOptions( GenericMultiLevelMap<string>& _optionLink )
{
...
void NonHoloEMA_Orth::setStandardOptions( GenericMultiLevelMap<string>& _optLink )
{
	_optLink["position_alpha"].as<double>() = 0.6;
	_optLink["angle_alpha"].as<double>() = 0.6;
	_optLink["switch_alpha"].as<double>() = 0.1;
	return;
}


Ptr<SceneObject::State> NonHoloEMA_Orth::newState( Mat& _state, RectangleRegion& /*_region*/, vector<Point>& /*_contour*/, Mat& /*_invImg*/, double _time )
{
	
	Angle conv(_state.at<float>(2));
	conv = conv.rad() + Angle::pi_half; // use orthogonal direction
	if( pHistory().size()!=0 )
	{
...
bool NonHoloEMA_Orth::addState( Ptr<SceneObject::State> _newState, RectangleRegion& _regionOfInterest, vector<Point>& _contour )
{
...
Mat NonHoloEMA_Orth::predictState()
{
...
void NonHoloEMA_Orth::resetPrediction()
{
	pDirectionSwitch = 0.5;
	pHasPredicted = false;
	return;
...
Point NonHoloEMA_Orth::predictPosition( Point _pt )
{
	predict();
...
Point NonHoloEMA_Orth::predictCtrPosition()
{
...
Point2f NonHoloEMA_Orth::predictCtrFPosition()
{
	if( pHistory().size()==0 ) return Point(0,0);
...
double NonHoloEMA_Orth::predictXCtrVelocity()
{
...
double NonHoloEMA_Orth::predictXCtrAcceleration() // since these Dynamics assume constant velocities, this function should return 0. Instead however, the acceleration given by the smoothened states is calculated and returned, assuming constant acceleration
{
	return accX();
}

double NonHoloEMA_Orth::predictYCtrVelocity()
{
	if( pHistory().size()==0 ) return 0;
...
double NonHoloEMA_Orth::predictYCtrAcceleration() // since these Dynamics assume constant velocities, this function should return 0. Instead however, the acceleration given by the smoothened states is calculated and returned, assuming constant acceleration
{
	return accY();
}

double NonHoloEMA_Orth::predictAgl()
{
	if( pHistory().size()==0 ) return 0;
	predict();
...
double NonHoloEMA_Orth::predictAglVelocity()
{
	if( pHistory().size()==0 ) return 0;
...
double NonHoloEMA_Orth::predictAglAcceleration() // since these Dynamics assume constant velocities, this function should return 0. Instead however, the acceleration given by the smoothened states is calculated and returned, assuming constant acceleration
{
	return accAng();
}

double NonHoloEMA_Orth::predictArea()
{
	return area();
}


void NonHoloEMA_Orth::drawDirection( Mat& _img, Scalar _color )
{
...
int NonHoloEMA_Orth::registerFactoryFunction()
{
	string info = "Nonholonomic movement where the direction of movement is orthogonal to the object axis. Smoothed positions with an exponential average filter. Prediction based on constant velocity assumption.";
	GenericMultiLevelMap<string> options;
	setStandardOptions(options);
	int _classId = Dynamics::registerDynamics( DynamicsFacEntry( "NonHoloEMA_Orth", &sfmCreator, info, options ) );
    return _classId;
}



int NonHoloEMA_Orth::classId = registerFactoryFunction();
