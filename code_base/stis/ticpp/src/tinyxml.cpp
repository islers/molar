...
bool TiXmlDocument::LoadFile( const char* _filename, TiXmlEncoding encoding )
{
	...
	*/
	...
	const char* p = buf;
	//std::cout<<std::endl<<"Test: "<<readData<<std::endl;
	buf[length] = 0;
	while( *p ) {
		assert( p < (buf+length) );
		if ( *p == 0xa ) {
...
bool TiXmlDocument::LoadFile( FILE* /*_file*/, TiXmlEncoding encoding )
{
    // unused version...
	std::ifstream file("test.xml",std::ios_base::in);
